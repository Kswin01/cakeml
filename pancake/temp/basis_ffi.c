...
void ffiget_time_input (unsigned char *c, long clen, unsigned char *a, long alen) {
  ...
  time++;
  res[0] = time / 4;
  res[1] = 0; // (time - 400) / 8; // 0 means no input; actual input n will be represented as n+1
  printf("get_time_input returns: time %ld and input %ld (input = 0 means no input)\n", res[0], res[1]);
  usleep(100 * 1000);
  return;
}

void ffi0 (unsigned char *c, long clen, unsigned char *a, long alen) {
  printf("Signal 0\n");
}

void ffi1 (unsigned char *c, long clen, unsigned char *a, long alen) {
  printf("Signal 1\n");
}

void ffi2 (unsigned char *c, long clen, unsigned char *a, long alen) {
  printf("Signal 2\n");
}
